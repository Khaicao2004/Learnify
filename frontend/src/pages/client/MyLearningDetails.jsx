          ...
          <div className="w-full max-w-lg rounded-xl bg-white shadow-lg border border-gray-100 overflow-hidden">
            {course?.sections.map((section, sectionIdx) => (
              <Disclosure as="div" key={section.id} defaultOpen={false}>
                    <DisclosureButton className="group flex w-full items-center justify-between px-4 py-3 bg-slate-50 hover:bg-slate-100 border-b border-gray-100 focus:outline-none">
                      <span className="font-medium text-slate-700">
                        Section {sectionIdx + 1}: {section.title}
                      </span>
                      <ChevronDownIcon className="h-5 w-5 text-slate-400 group-data-open:rotate-180" />
                    </DisclosureButton>
